...
pub fn run_server(
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
                                ...
                                select! {
                                    Some(x) = router_socket.next() => {
                                        ...
                                        let payload = match payload {
                                            KVSRequest::Put { key, .. } => KVSRequest2::Put {
                                                key,
                                                value: BufferPool::get_from_buffer_pool(&buffer_pool)
                                            },
                                            ...
                                            KVSRequest::Gossip { key, reg } =>
                                                ...
                                                KVSRequest2::Gossip {
                                                    key,
                                                    reg: MyRegType2 { val: BufferPool::get_from_buffer_pool(&buffer_pool), marker: reg.marker }
                                                }
                                            ,
                                        };
                                    ...
                                    async move {
                                        while let Ok((key, reg)) = transducer_to_peers_rx.recv().await {
                                            ...
                                            let req = KVSRequest::Gossip {
                                                key,
                                                reg: LWWReg {
                                                    val: ValueType{ data: reg.val.borrow().unwrap()[..].try_into().unwrap() },
                                                    marker: reg.marker,
                                                }
                                            };
            ...
            let pre_gen_random_numbers: Vec<u64> = (0..(128*1024)).map(|_| rng.sample(dist) as u64).collect(); // rust is really bad at dealing with arrays.
            ...
            let mut df = hydroflow_syntax! {
                ...
                client_input = my_merge
                    -> demux(|(req, addr): (KVSRequest2, Vec<u8>), var_args!(gets, store, broadcast)| {
                        match req {
                ...
                // join for lookups
                lookup = lattice_join::<'static, 'tick>();
