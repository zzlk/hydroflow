...
#[derive(Debug)]
pub struct HalfJoinStateLattice2<Key, LR: LatticeRepr + Merge<LRD>, LRD: LatticeRepr> {
    table: FxHashMap<Key, LR::Repr>,
    _marker: PhantomData<*const LRD>,
}

impl<Key, LR: LatticeRepr + Merge<LRD>, LRD: LatticeRepr> Default
    for HalfJoinStateLattice2<Key, LR, LRD>
{
    fn default() -> Self {
        Self {
...
impl<Key, LR: LatticeRepr + Merge<LRD> + Convert<LRD>, LRD: LatticeRepr>
    HalfJoinStateLattice2<Key, LR, LRD>
where
...
pub type _JoinStateLattice<Key, V1: LatticeRepr, V1D, V2: LatticeRepr, V2D> = (
    HalfJoinStateLattice2<Key, V1, V1D>,
    HalfJoinStateLattice2<Key, V2, V2D>,
);
pub type JoinStateLatticeMut<'a, Key, V1: LatticeRepr, V1D, V2: LatticeRepr, V2D> = (
    &'a mut HalfJoinStateLattice2<Key, V1, V1D>,
    &'a mut HalfJoinStateLattice2<Key, V2, V2D>,
);

pub struct SymmetricHashJoinLattice2<'a, Key, V1, V1D, V2, V2D>
where
...
impl<'a, Key, V1, V1D, V2, V2D> Iterator for SymmetricHashJoinLattice2<'a, Key, V1, V1D, V2, V2D>
where
    Key: Eq + std::hash::Hash + Clone,
...
impl<'a, Key, V1, V1D, V2, V2D> SymmetricHashJoinLattice2<'a, Key, V1, V1D, V2, V2D>
where
    Key: Eq + std::hash::Hash + Clone,
    V1: Merge<V1D> + Convert<V1D>,
    V1::Repr: Eq + Clone,
    ...
    V2D::Repr: Eq + Clone,
{
    pub fn new_from_mut<I1, I2>(
        mut lhs: I1,
        mut rhs: I2,
        state_lhs: &'a mut HalfJoinStateLattice2<Key, V1, V1D>,
        state_rhs: &'a mut HalfJoinStateLattice2<Key, V2, V2D>,
    ) -> Self
    where
...
#[cfg(test)]
mod tests {
    use super::{SymmetricHashJoinLattice2, _JoinStateLattice as JoinStateLattice};
    ...
    fn join<X: IntoIterator<Item = (usize, usize)>, Y: IntoIterator<Item = (usize, usize)>>(
        ...
        SymmetricHashJoinLattice2::new_from_mut(
            lhs.into_iter(),
            rhs.into_iter(),
            &mut state.0,
            &mut state.1,
